...
def run():
  ...
  with grpc.insecure_channel("localhost:50051") as channel:
    # submit调度可调用对象fn，以fn(*args ** kwargs)方式执行并返回Future对象代表可调用对象的执行
    ...
    #
    # result(timeout=None)返回调用返回的值。如果调用还没完成那么这个方法将等待timeout秒。如果在timeout秒内没有执行完成，concurrent.futures.TimeoutError
    # 将会被触发。timeout可以是整数或浮点数。如果timeout没有指定或为None，那么等待时间就没有限制。
    # 如果futrue在完成前被取消则CancelledError将被触发。
